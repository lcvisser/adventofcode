...
# Decay table (source: https://njohnston.ca/2010/10/a-derivation-of-conways-degree-71-look-and-say-polynomial/)
# # Subsequence Length Evolves Into
element_table_source = """
1   1112    4   (63)
2   1112133 7   (64)(62)
...
DECAY_TO = re.compile(r"\(([0-9]+)\)")

element_table = dict()
for row in element_table_source.strip().split("\n"):
    element, sequence, length, decay = row.split()
    decay_elements = DECAY_TO.findall(decay)
    element_table[int(element)] = (sequence, int(length), [int(e) for e in decay_elements])

def find_element(s):
    element = None
    for element, (sequence, _, _) in element_table.items():
        if sequence == s:
...
def decay(start_element, num_iter):
    counter = collections.Counter({start_element: 1})
    for _ in range(num_iter):
        new_counter = collections.Counter()
        for e in counter.elements():
            new_counter.update(element_table[e][2])  # decay elements

        counter = new_counter

    count = 0
    for e in counter.elements():
        count += element_table[e][1]  # sequence length

    return count


start_element = find_element("3113322113")

# Part 1: after 40 iterations
print(f"Length after 40 iterations: {decay(start_element, 40)}")

# Part 2: after 50 iterations
print(f"Length after 50 iterations: {decay(start_element, 50)}")
